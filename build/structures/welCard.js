...
class welCard {
    ...
    async build() {
        if(!this.name) { throw new Error("Nissing name parameter"); }
        ...
        let validatedColor = await colorFetch(
            this.color || "ff0000",
            parseInt(this.brightness) || 0,
            thumbnailURL
        )

        const img = await canvas.loadImage(`${devmod ? "" : "node_modules/welcard/"}res/card.png`);
